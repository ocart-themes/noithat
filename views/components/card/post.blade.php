...
@if($data)
    <div
        class="card-post {{ $class }} @if($type === 'thumb-right' || $type === 'thumb-left') flex @endif @if($type === 'thumb-bottom') flex flex-col-reverse @endif @if($type === 'thumb-bg') thumb-bg flex relative @endif h-full block overflow-hidden transition ease-in-out duration-500 transform @if($transform) hover:-translate-y-3 @endif"
    >
        @if($thumbnail)
            <div
        ...
        >
            ...
            @if($postMeta)
                <div class="@if($dark || $type === 'thumb-bg') text-white @else text-gray-600 @endif text-sm post-meta">
                    @if($author)
                        <span class="mr-2">
                    Admin
                </span>
